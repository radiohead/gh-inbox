...
| Package/Module | Purpose | Key Dependencies |
|----------------|---------|------------------|
| `cmd/prs/review` | CLI handler: flag parsing, filter resolution, pipeline wiring | service, github, output |
| `cmd/prs/authored` | CLI stub: placeholder for "my PRs need response" feature | service, github, output |
| `internal/service/pipeline` | Core pipeline orchestration: Fetch → Classify → Filter → Output | github interfaces (Fetcher) |
| `internal/service/classify` | Three-axis PR classification: ReviewType, AuthorSource, ReviewStatus | github types, cache |
| `internal/service/filter` | PR filtering with presets (focus, nearby, org, all) and granular flags | service types |
| `internal/service/team` | Team membership cache and queries: SharesTeamWith, IsTeamMember | github, cache |
| `internal/github/client` | GitHub API abstraction: GraphQL + REST with optional disk caching | cli/go-gh, shurcooL-graphql |
| `internal/github/prs` | PR fetching via GraphQL search and review request parsing | github/client |
| `internal/github/team_members` | Current user and team member fetching via REST API | github/client |
| `internal/github/review_requests` | Review request structure and classification helpers | github types |
| `internal/output/table` | Human-readable table output with TTY-aware coloring | stdlib (io, text/tabwriter) |
| `internal/output/json` | Machine-readable JSON output formatting | stdlib (encoding/json) |
| `internal/cache` | Disk-based caching: SHA-256 keys, atomic writes, mtime-based TTL | stdlib (crypto, os) |
| `internal/errors` | Error severity classification for graceful degradation (Critical/Warning/Silent) | github types |

**Technology Stack**: Go 1.26, Cobra (CLI), cli/go-gh (GitHub auth), shurcooL-graphql (GraphQL client), golang.org/x/sync/errgroup (concurrency), golangci-lint 2.10.1 (linting)

**Architecture Pattern**: Layered pipeline (Fetch → Classify → Filter → Output) with strict unidirectional dependencies, narrow interfaces at consumption points, and codified invariants in CONSTITUTION.md
...
- [docs/architecture/README.md](docs/architecture/README.md) — Architecture records, component details, and design rationale
- [CONSTITUTION.md](CONSTITUTION.md) — Codified architectural invariants (dependency rules, pipeline ordering, failure policies)
